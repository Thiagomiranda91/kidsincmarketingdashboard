...
## Contact Form page (Flamingo)

`/contact-form` (linked from the nav bar as "Contact Form") lists all
Contact Form 7 submissions in the selected time window, pulled from the
Flamingo plugin on your WordPress site: date, name, email, and subject.
Message content isn't shown inline — click "View message" on any row to
open it in a popup. Filter by the same 7/14/30-day + custom range picker
used elsewhere in the dashboard.

**Important:** Flamingo has no built-in REST API or public data feed — it
just stores submissions in the WordPress database. To make this page work,
you need to install a small companion plugin on your WordPress site first.
See "Setting up the Flamingo bridge" below.


## PDF report

The green **Generate Report** button in the nav bar opens `/report`, a
combined PDF-ready report covering Website Overview, Facebook, Instagram,
and Contact Form Submissions for a selected date range (7/14/30 days —
custom ranges aren't supported on this page, only the three presets).

It works by reusing all the existing API routes (no new backend endpoints)
and rendering them into a clean, light-background, print-optimized layout —
deliberately not the dashboard's dark theme, since printing a dark UI wastes
ink and reads poorly on paper. Click **Print / Save as PDF**, which opens
your browser's native print dialog; choose "Save as PDF" as the destination
there to get an actual PDF file.

This intentionally does not use a server-side PDF renderer (e.g. headless
Chrome via Puppeteer) — that approach needs a Chromium binary bundled into
serverless functions, adds real deployment weight, and can bump into
Vercel's function time limits on lower tiers. The browser-print approach has
none of those constraints and needs zero extra dependencies.
...
## 5. Setting up the Flamingo bridge

Flamingo (the plugin that stores Contact Form 7 submissions) doesn't expose
any API on its own, so this repo includes a small snippet that adds one
secured, read-only endpoint to your WordPress site. There are two ways to
install it — pick whichever is easier for you:

**Option A — functions.php** (`wordpress/flamingo-api-bridge-functions-php.php`)
Paste the contents into your theme's `functions.php`. Simpler to manage, but
if you're not using a child theme, updating the parent theme later will wipe
it out and you'll need to re-add it. This version has the API key defined
directly in the snippet (see step 3 below) since many hosts don't give
theme-only access to `wp-config.php`.

**Option B — standalone plugin** (`wordpress/flamingo-api-bridge.php`)
Upload it to `/wp-content/plugins/flamingo-api-bridge/` and activate it under
Plugins. Slightly more setup, but survives theme updates/switches. This
version expects the API key in `wp-config.php` — use this if you do have
access to it, since keeping secrets out of theme code is generally better
practice.

1. Confirm both **Contact Form 7** and **Flamingo** are already active on your WordPress site.
2. Add the snippet using Option A or B above.
3. Set the API key:
   - **Option A:** open the pasted snippet in `functions.php` and replace
     `'replace-with-a-long-random-string'` with a real generated value.
   - **Option B:** open `wp-config.php` and add, above the `/* That's all, stop editing! */` line:
     ```php
     define('FLAMINGO_BRIDGE_API_KEY', 'replace-with-a-long-random-string');
     ```
   Either way, generate the value the same way as the other secrets in this project:
   ```bash
   openssl rand -hex 32
   ```
   **Note:** with Option A, anyone who can edit your theme files (via the
   WordPress theme editor, FTP, hosting file manager, etc.) can read this key
   in plain text. That's usually just site admins, but it's a lower bar than
   wp-config.php, which is typically more restricted. Don't commit this file
   with the real key to a public GitHub repo if your theme is version-controlled.
4. Set two env vars on Vercel (and in `.env.local` if running locally):
   ```
   FLAMINGO_API_URL=https://yoursite.com/wp-json/flamingo-bridge/v1/messages
   FLAMINGO_API_KEY=<the exact same string you used in step 3>
   ```
5. Redeploy. The dashboard authenticates by sending that key in an `X-API-Key`
   header — anyone without it gets a 401, so the endpoint is safe to expose
   publicly on your WordPress site.

The endpoint only returns non-spam, non-trashed messages, newest first, and
supports `?since=` and `?until=` query params (`YYYY-MM-DD`) matching the
time-window filter on the Contact Form page. With no date params it returns
everything.

**Message content** shown in the "View message" popup isn't the raw form
submission — it's rebuilt from these specific field names on your Contact
Form 7 form (the bridge reads them from Flamingo's stored `_fields` data):

```
county
school-clare / school-cork / school-dublin / school-galway / school-kildare /
  school-limerick / school-meath / school-sligo / school-waterford /
  school-westmeath / school-wexford / school-wicklow  (whichever one the form shows)
programme-standard / programme-dublin  (whichever one the form shows)
parent-name
phone-number
email-address
child-dob-formatted
ideal-start-date-formatted
days-required  (checkbox field — each checked day is listed on its own line)
breakfast-club
```

If your CF7 field `name` attributes don't match these exactly, that field
will just show up blank in the popup — double check the field names in your
form editor if something's missing.

**Technical note:** the bridge reads each field from WordPress's individual
`_field_<name>` post meta entries (e.g. `_field_county`), not from Flamingo's
combined `_fields` array — Flamingo's own save logic blanks every value in
that array to null right before storing it, so it's unusable for this
purpose. This is a quirk of Flamingo itself, not a bridge bug.
...
## 7. Deploying to Vercel via GitHub
...
pages/
  ...
  contact-form.js               — Contact Form 7 submissions (via Flamingo)
  report.js                    — combined print/PDF report (Overview + Facebook + Instagram + Contact Form)
  ...
  api/
    ...
    flamingo-messages.js       — recent Contact Form 7 submissions
...
components/
  ...
  Nav.jsx                      — top nav bar (Overview / Facebook / Instagram / Contact Form + sign out)
...
lib/
  ...
  flamingoClient.js            — client for the WordPress Flamingo bridge endpoint
  format.js                    — date/label formatting helpers
  dateRange.js                 — time-window filter state, resolving to startDate/endDate
  theme.js                     — shared dark-theme design tokens (colors, fonts, radius)
wordpress/
  flamingo-api-bridge.php              — standalone plugin option (see setup section above)
  flamingo-api-bridge-functions-php.php — functions.php snippet option (see setup section above)
public/
