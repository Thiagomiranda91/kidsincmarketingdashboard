...
it out and you'll need to re-add it.
...
Plugins. Slightly more setup, but survives theme updates/switches.
...
3. Open your site's `wp-config.php` and add, above the `/* That's all, stop editing! */` line:
   ```php
   define('FLAMINGO_BRIDGE_API_KEY', 'replace-with-a-long-random-string');
   ```
   Generate a value the same way as the other secrets in this project:
   ```bash
   openssl rand -hex 32
   ```
4. Set two env vars on Vercel (and in `.env.local` if running locally):
   ...
   FLAMINGO_API_KEY=<the same string you put in wp-config.php>
   ```
