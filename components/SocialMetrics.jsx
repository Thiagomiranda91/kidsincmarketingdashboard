import { UserPlus, UserMinus, FileText, ThumbsUp, Flame, Eye, Users, DollarSign, MousePointerClick } from "lucide-react";
...
export default function SocialMetrics({ platformName, brandColor, metrics, isLive, range, onRangeChange }) {
  const netFollowers = metrics.netFollowers || 0;
  const isPositive = netFollowers >= 0;
    ...
    <div style={{ fontFamily: "'Inter', -apple-system, sans-serif", background: PAPER, color: INK, minHeight: "100vh" }}>
      ...
      <div style={{ padding: "32px 28px 48px" }}>
        ...
        <div style={{ display: "flex", gap: 14, flexWrap: "wrap" }}>
          <KPICard
            icon={isPositive ? UserPlus : UserMinus}
            eyebrow="Net Followers"
            value={`${isPositive ? "+" : ""}${fmtNum(netFollowers)}`}
            accent={isPositive ? "#12704F" : "#B4402A"}
            sub="gain / loss"
          />
