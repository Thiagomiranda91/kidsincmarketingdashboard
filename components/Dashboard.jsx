...
import { Users, UserPlus, Clock, Calendar, Target } from "lucide-react";
...
export default function Dashboard({ visitors, newReturning, topPages, searchQueries, channels, keyEvents, isLive }) {
    ...
    <div style={{ fontFamily: "'Inter', -apple-system, sans-serif", background: PAPER, color: INK, minHeight: "100%", padding: "32px 28px 48px" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", flexWrap: "wrap", gap: 16, marginBottom: 28, borderBottom: `2px solid ${INK}`, paddingBottom: 18 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 13, color: "#5C5748", fontFamily: "'JetBrains Mono', monospace" }}>
          <Calendar size={14} />
          Traffic: 6-month trend · Engagement &amp; search: last 28 days
        </div>
      </div>

      {/* KPI strip */}
      <div style={{ display: "flex", gap: 14, flexWrap: "wrap", marginBottom: 24 }}>
        <KPICard icon={Users} eyebrow="Unique Visitors" value={fmtNum(currentMonth.visitors)} sub={`${currentMonth.month} · monthly`} />
        <KPICard icon={UserPlus} eyebrow="New Users" value={fmtPct(newPct)} sub={`${fmtNum(newUsers)} of ${fmtNum(totalNewReturning)} · 28 days`} />
        <KPICard icon={UserPlus} eyebrow="Returning Users" value={fmtPct(returningPct)} sub={`${fmtNum(returningUsers)} of ${fmtNum(totalNewReturning)} · 28 days`} />
        <KPICard icon={Clock} eyebrow="Avg. Time on Page" value={fmtTime(overallAvgTime)} sub="weighted across top pages · 28 days" />
        <KPICard icon={Target} eyebrow="Key Events" value={fmtNum(keyEvents.total)} sub="28 days · was 'Conversions'" />
      </div>
      ...
      <div style={{ display: "flex", gap: 20, marginBottom: 20, flexWrap: "wrap" }}>
        <Card title="New vs. Returning Users" sourceLabel="28 DAYS" style={{ flex: 1, minWidth: 280 }}>
          <div style={{ display: "flex", height: 28, borderRadius: 3, overflow: "hidden", marginBottom: 12 }}>
            <div style={{ width: `${newPct}%`, background: TEAL }} />
            <div style={{ width: `${returningPct}%`, background: ACCENT }} />
          </div>
        ...
        <Card title="% of Sessions by Channel" sourceLabel="28 DAYS" style={{ flex: 1, minWidth: 280 }}>
          {channels.map((c, i) => (
        ...
        <Card title="Key Events by Type" sourceLabel="28 DAYS" style={{ flex: 1, minWidth: 280 }}>
      ...
      {/* Top pages */}
      <Card title="Top Pages — Views &amp; Engagement" sourceLabel="28 DAYS · SOURCE: GA4" style={{ marginBottom: 20 }}>
      ...
      {/* Search queries */}
      <Card title="Top Search Queries (Google)" sourceLabel="28 DAYS · SOURCE: Search Console">
        <div style={{ display: "flex", fontSize: 11, color: "#8A8371", fontFamily: "'JetBrains Mono', monospace", textTransform: "uppercase", padding: "0 0 8px", borderBottom: `1px solid ${LINE}` }}>
