...
// Net follower change over a window, using a running-total metric.
// Facebook Pages: metric = "page_follows"
// Instagram:      metric = "follower_count"
export async function fetchNetFollowerChange({ objectId, metric, since, until }) {
...
export async function fetchFacebookPostStats({ pageId, since, until }) {
  let posts = [];
  let data = await graphGet(`${pageId}/posts`, {
