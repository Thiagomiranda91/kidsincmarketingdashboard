    ...
    function flamingo_bridge_get_messages(WP_REST_Request $request)
    {
        ...
        foreach ($query->posts as $post) {
            $messages[] = [
                ...
                'fields'    => flamingo_bridge_format_fields($post->ID),
            ];
        }
...
/**
 * Builds the list of { label, value } pairs shown in the dashboard's
 * "View message" popup, from only the specific submitted form fields the
 ...
 * before writing it (see class-inbound-message.php in the Flamingo source).
 * The real per-field values live in individual '_field_<name>' post meta
 * entries instead, which is what this reads.
 */
...
if (!function_exists('flamingo_bridge_get_field')) {
...
if (!function_exists('flamingo_bridge_format_fields')) {
    function flamingo_bridge_format_fields($post_id)
    {
        ...
        return [
            ...
            ['label' => 'Child Date of Birth', 'value' => $dob],
            ['label' => 'Ideal Start Date', 'value' => $start_date],
            ['label' => 'Days Required', 'value' => $days_required],
            ['label' => 'Breakfast Club', 'value' => $breakfast_club],
        ];
    }
}
